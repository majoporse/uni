...
class Warehouse:
    ...
    def store(self, item: str, amount: int,
        ...
        i = 0
        while i < len(package_list) and \
                int(expiry) < int(package_list[i].expiry):
            i += 1
    ...
    def make_inventory(self, history: List['Movement']) -> \
        ...
        for movement in history:
            ...
            inventory[item][price] = \
                inventory[item].get(price, 0) + amount

        return inventory

    def find_inconsistencies(self) -> Set[Tuple[str, int, int]]:
        history = self.history
        result = self.make_inventory(history)
        for item in self.inventory:
            ...
            for package in self.inventory[item]:
                ...
                if price not in result[item].keys():
                    result[item][price] = 0

                result[item][price] -= amount

        return {(item, price, -result[item][price])
                for item in result
                for price in result[item]
                if result[item][price] != 0}
    # v haskeli sme to teraz
    # mali tak som vyskusal :)

    def remove_package(self, package: 'Package',
                       item: str, message: str) -> None:

        self.history += [Movement(item,
                                  - package.amount,
                                  package.price, message)]
    ...
    def remove_expired(self, today: str) -> List['Package']:
        ...
        for item in inventory:
            for package in inventory[item].copy():
                if int(package.expiry) < int(today):
                    expired.append(package)
                    self.remove_package(package, item, 'EXPIRED')
        return expired
    ...
    def average_prices(self) -> Dict[str, float]:
        ...
        for item in inventory:
            avg = 0
            amount = 0
            for package in inventory[item]:
                avg += package.amount * package.price
                amount += package.amount
            if amount != 0:
                result[item] = (avg / amount)
        return result

    def best_suppliers(self) -> Set[str]:
        history = self.history
        item_suppliers: Dict[str, Dict[str, int]] = {}
        for movement in history:
            ...
            if amount > 0:
                suppliers = item_suppliers.get(item, {})
                suppliers[name] = suppliers.get(name, 0) + amount
                item_suppliers[item] = suppliers

        result: Set[str] = set()

        for item in item_suppliers:
            best = 0
            best_suppliers = []
            for supplier in item_suppliers[item]:
                current = item_suppliers[item][supplier]
                if current == best:
                    best_suppliers.append(supplier)

                if current > best:
                    best = item_suppliers[item][supplier]
                    best_suppliers = [supplier]

            result = result.union(set(best_suppliers))
        return result

    def packages_to_sell(self, inventory: List['Package'], amount: int,
        ...
        for index in range(-1, -len(inventory) - 1, -1):
            package_quantity = inventory[index].amount
            package_value = inventory[index].price

            if total_amount + package_quantity > amount:
                break

            total_price += package_quantity * package_value
            total_amount += package_quantity
            ...
            if index == -len(inventory):
                return (to_remove, best_amount, best_price, 0)
        # idea bola, ze ak sa mi uz nezmestia baliky do amount, tak
        # sa mi loop breakne a checknem, ci, ked pridam leftovers tak sa mi
        # priemerna cena zmensi pod price. upravil som loop pretoze sa mi
        # nenechala v ISe moja najnovsia verzia, ktora mi failla.
        leftovers = amount - total_amount
        if (total_price + leftovers * package_value) / \
           (total_amount + leftovers) <= price:

            best_price, best_amount = total_price + leftovers * package_value,\
                                    total_amount + leftovers
            if leftovers > 0:
                to_remove = index + 1
                inventory[index].amount -= leftovers

        return (to_remove, best_amount, best_price, leftovers)

    def remove_packages(self, to_remove: int, inventory: List['Package'],
    ...
    def find_leftovers(self, index: int, best_amount: int, best_price: int,
                       parameters: Tuple[List[Package], int, str, str]) \
            -> Tuple[int, int]:
    ...
    def try_sell(self, item: str, amount: int,
        ...
        if inventory == [] or amount == 0:
            return (0, 0)

        to_sell, best_amount, best_price, leftovers = \
            self.packages_to_sell(inventory, amount, price)
