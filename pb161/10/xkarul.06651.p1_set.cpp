...
/** Drobnosť ohľadom formátovania: je fajn dávať prázdne riadky
 ** medzi deklaráciou atribútov, konštuktorov a metód, naozaj
 ** to pomáha čitaťeľnosti **/
struct set{
    std::set<int> data;
    void add(int i) {data.emplace(i);}
    bool has(int i) const {return data.contains(i);}
    set(const set&) = default;
    /** Tu mohol byť použitý std::move takto "data{std::move(new_data)}" **/
