...
class grammar{
public:
    /** std::string neni zrovna nejvhodnejsi zpusob reprezentace,
     ** radsi bych pouzil tuple/pair (taky se radi lexikograficky). **/
    ...
    void generate_rec( char a, int b, std::vector<std::string> &res, std::string s = "") const{
        if (b == 0)
            return;
        --b;
        /** Zvlast contains() a pak pristup zbytecne hleda prvek dvakrat,
         ** lepsi pouzit find(). **/
        if (!rules.contains(a))
            return;
