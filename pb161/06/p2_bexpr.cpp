...
{
public:
    virtual bool eval(const input & /*unused*/) = 0;
    node() = default;
}; /* ref: 6 lines */
...
public:
    ...
    bool eval(const input &i) override
    {
        bool a = left.eval(i);
        bool b = right.eval(i);
        return (num & (1 << (2 * a + b))) > 0;
    }

    operation(uint16_t i, node &n1, node &n2) : left{n1},
                                                right{n2},
                                                num{i} {}
}; /* ref: 16 lines */
...
public:
    bool eval(const input &i) override { return i.at(value); }
    variable(int i) : value{i} {}
}; /* ref: 7 lines */
