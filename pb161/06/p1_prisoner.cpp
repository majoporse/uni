...
public:
    bool betray(bool /*unused*/) override{
        return true;
    }
};
...
int play( prisoner &a, prisoner &b, int rounds ){
    ...
    for (bool round_a, round_b; rounds > 0; --rounds){
        round_a = a.betray(last_move);
        round_b = b.betray(last_move);
        if (round_a == round_b){
            count_a += round_a + 1;
            count_b += round_b + 1;
        } else if (round_b)
