...
public:
    ...
    virtual bool intersects(const object &a) const;

    /** Nice, never forget about virtual destructor!
     ** (as I did)
     ** (unless it is changed to be implicit in the future) **/
...
struct circle : public object{
    point s;
    double r;
    bool intersects(const circle &a) const override {
        double d = dist(s, a.s);
        /** This would have looked nicer if you switched call
         ** if, for example, you wanted left side to be bigger,
         ** so you would do "if (r < a.r) return a.intersects(*this);"
         **/
        const circle &larger = r > a.r ? *this : a;
        const circle &smaller = !(r > a.r) ? *this : a;

        return d >= larger.r ? (d <= r + a.r) : (d + smaller.r >= larger.r);
    }
    bool intersects(const line &l) const override{
        return dist(l, s) <= r;
    }
    circle(point s, double r) : s{s}, r{r}{}
...
public:
    ...
    bool intersects(const line &l) const override{
        /* The <slope> struct would have saved you some time
         * and cognitive effort here */
        return (close(dist(l, p), 0) &&
                close(dist(l, q), 0)) ||
               !close(std::tan((p.first - q.first) / (p.second - q.second)),
                      std::tan((l.p.first - l.q.first) / (l.p.second - l.q.second)));
    }
    line(point p, point q) : p{p}, q{q}{}

};   /* ref: 18 lines */

/** This could have been implemented in the subclasses using one line:
 ** "return a.intersects(*this);" thanks to dynamic binding.
 ** For example, if we look at struct <circle> and implement there
 ** <intersects(const object &o)>, we know that if we call
 ** <o.intersects(*this)> that <*this> is of type <circle> which we have
 ** already implemented before in other subclasses. **/
/* Definitions of the helper functions. */
bool object::intersects(const object &a)const{
        if(dynamic_cast<const circle *>(&a)){
            auto *s = dynamic_cast<const circle *>(&a);
            return intersects(*s);
        }
        auto *s = dynamic_cast<const line *>(&a);
        return intersects(*s);
}
