 ...
 **   * naming impacts this point a lot, be careful, cause it might slip to major
 ** issues :/
 ** * [x] correctly decomposes the problem
 **   * there are some questionable parts, e.g. `erase → erase → del`…
 ** * [x] doesn't have code duplication
 ** * [!] uses correct abstractions, algorithms and data structures
 **   * assignment requires copy&move for A, sorry about that
 **
 ** ---
 **
 ** Summary (+1s, suggestions, rants, confessions, etc.):
 ** * #rant Can you please use empty lines to separate stuff and blocks of code that
 **  doesn't make sense to factor out to their own functions? Just have a look at
 ** the `node`, it's one »dense« structure, members, functions, everything “glued”
 ** together without »one« empty line…
 **   Examples:
 **   * https://blog.mfocko.xyz/files/blog/leetcode/sort-matrix-diagonally/matrix-
 ** sort.cpp
 **   * https://gitlab.com/mfocko/LeetCode/-/blob/main/problems/boats-to-save-
 ** people.cpp (even if there's barely anything it might make sense to separate it)
 **
 ** ---
 **
...
struct treap{
/** I'd expect the treap to rot in the destructor, but OK. **/
...
/** `s` what? **/
    ...
    std::pair<treap_node *, status> search(int key)const{
        ...
        while ( cur->k != key){
            if (key < cur->k){
                if (cur->l == nullptr)
/** I guess that you could deduce the status from the pointer that is returned, but
 ** OK. **/
...
/** Did you test it with the property-based tests? :poggies: **/
...
/** I don't really like this, but I have a feeling that I know _why_ you did it like
 **  this :/ **/
...
/** pre(fix)? **/
    treap_node *get_pre(treap_node *ptr){
        if(ptr->l == nullptr)
            return ptr;
        ptr = ptr->l.get();
        while( ptr->r != nullptr) ptr = ptr->r.get();
        return ptr;
    }

/** Just do `__del__` and we're back in Python ‹insert Vietnam flashbacks›. **/
    void del(treap_node *node){
        if(rot.get() == node)
            rot.reset();
        else
    ...
    void erase(treap_node *node){
        ...
        if (!node->l && !node->r)
/** +1 for the decomposition, but please give it some reasonable name :/ **/
            del(node);
...
/** Allow `search` to handle `root() == nullptr`… it will be much cleaner than this **/
        if (root() == nullptr)
            return false;
        auto[ptr, stat] = search(key);
...
/** This fails horribly for empty tree :/ Follow the suggestion above :D **/
        auto[ptr, stat] = search(key);
...
/** See? You can use vertical spacing and even reasonably! USE IT MORE :slams the
 ** desk: :D **/
