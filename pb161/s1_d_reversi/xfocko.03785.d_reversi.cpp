 ...
 **   * Please switch either to `map` or 1D-vector, that nested structure is not
 ** really ideal in this case, you need to keep in mind that it's not raw memory and
 **  each of the nested vectors have their own “background”.
 **
 ** ---
 **
...
using square = std::vector<line>;
using cube = std::vector<square>;
...
/** So what happens if start gets called multiple times? If you wanted to keep them
 ** precomputed, either use a function from which you save it as a `const` or
 ** encapsulate it within your class/struct (it can be private and static) **/
...
struct reversi
{
/** It's not very ideal solution, I would recommend using `std::map<position,
 ** color>` or 1D vector. **/
    cube play_space;
    bool _turn;
    int _xs, _ys, _zs;
...
/** Nice! **/
        for (int dz = 0; dz <= 1; ++dz, t = !t){
            for (int dy = 0; dy <= 1; ++dy, t = !t){
                for (int dx = 0; dx <= 1; ++dx, t = !t){
                    ...
                    play_space[z+dz][y+dy][x+dx] = t ? WHITE : BLACK;
                }
            }
        }
    }
    ...
    int check_full_line(position pos, direction d, int turn) const {
        ...
        while (is_valid_position(x, y, z) &&
                play_space[z][y][x] == opponent_stone){
                ++len;
                x += dx;
                y += dy;
                z += dz;
        ...
        if (!is_valid_position(x, y, z) || (len > 0 && play_space[z][y][x] != end))
            return 0;
...
/** ```suggestion:-0+0
 **     bool available_play(bool turn) const{
 ** ``` **/
    bool avalible_play(bool turn) const{
        ...
        for (int z = 0; z < _zs; ++z){
            for (int y = 0; y < _ys; ++y){
                ...
                if(play_space[z][y][x] == my_stone && has_play({x, y, z}, turn))
                    return true;
                }
            }
        }
    ...
    bool playable(position pos) const{
        auto [x, y, z] = pos;
        return is_valid_position(x, y, z) && play_space[z][y][x] == EMPTY
               && has_play(pos, _turn);
    }
...
/** You don't really need to keep count, you can sum it up at the end, but OK. **/
        ...
        for (;len > 0 ; --len, x += dx, y += dy, z += dz){
            if (play_space[z][y][x] == opponent_colour)
                oopponent_count--;
            play_space[z][y][x] = colour;
            player_count++;
        }
    }
    ...
    void start( int x_size, int y_size, int z_size ){
        ...
        for (int z = 0; z < z_size; ++z){
            square layer(y_size,line(x_size, EMPTY));
            play_space.push_back(layer);
        }
    ...
    bool play( int x, int y, int z ){
        ...
        fill_position(pos);
        play_space[z][y][x] = _turn ? WHITE : BLACK;
        _turn = !_turn;
        return true;
    }
