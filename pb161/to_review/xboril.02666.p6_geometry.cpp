...
struct slope : std::pair<double, double> // smerovej vektor proste
{
...
public:
    ...
    {
        double dst = dist(p, c.p);
        if (dst > radius + c.radius)
        {
            return false;
        }
        if (dst >= std::abs(radius - c.radius))
        {
            return true;
        }
        return false;
