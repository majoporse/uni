...
class node{
    public:
        virtual bool eval(const input& vars) = 0; 
}; /* ref: 6 lines */
...
class operation : public node{
    ...
    public:
    operation(unsigned int table, node& a, node& b) : left{a}, right{b}, table{table}{
    }
    bool eval(const input& vars) override{
        bool left_val = left.eval(vars);
        bool right_val = right.eval(vars);
