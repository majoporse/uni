...
struct numset {
    ...
    bool add(int i) { return nset.insert(i).second; }

    bool del(int i) { return nset.erase(i); }

    bool has(int i) const { return nset.contains(i); }

    void del_range(int i, int j) {
        for (; i <= j; i++) this->del(i);
    }

    void merge(numset ns2) {
        for (int n : ns2.nset) this->add(n);
    }
};
...
int main() {
    ...
    for (int n : {1, 3, 5, 7, 11, 17}) assert(a.add(n));
    for (int n : {2, 4, 8, 16}) assert(b.add(n));
