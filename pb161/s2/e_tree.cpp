...
void set_unique(node_ptr&,const node*);
...
struct tree{
    ...
    tree() = default;
    tree(node_ptr r): root{std::move(r)}{}
    tree(const tree&t){
        set_unique(root, t.root.get());
    }
    tree(tree&&t){
...
struct node_object : public node {
    ...
    void take(int i, tree &t)  override { take(i, std::move(t)); }
...
void set_unique(node_ptr&plac, const node *p){
    if (!p)
        plac = nullptr;
    else if (p->is_array())
