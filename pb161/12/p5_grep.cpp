...
void grep( std::string pattern, std::istream &in, std::ostream &out){
    std::string s;
...
void cut( char delim, int col, std::istream &in, std::ostream &out ){
    std::string s;
    while ( std::getline(in, s)){
        std::istringstream in2(s);
        std::string s2;
        for (int i = 0; std::getline(in2, s2, delim); ++i){
            if (i == col){
                break;
            }
            s2.clear();
