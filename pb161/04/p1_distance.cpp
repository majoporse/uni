...
struct walk{
    ...
    std::tuple<double, double> _current = {0, 0};

    double _dist = 0;

    std::tuple<double, double> _last = {0, 0};

    double _last_dist = 0;

    walk() = default;
    walk(std::tuple<int, int> p) : _current{p} {}

    double pdistance(std::tuple<double, double> pos){
        auto [x, y] = pos;
        auto [xc, yc] = _current;
        return std::sqrt((x-xc)*(x-xc) + (y-yc)*(y-yc));
    }

    walk &line(std::tuple<double, double> p){
        double d = pdistance(p);
    ...
    walk &arc(std::tuple<double, double> p, double radius){
        double d = pdistance(p) / 2;
        double angle = std::asin(d / radius) * 2;

         _last = _current;
        _current = p;

        _last_dist = _dist;
        _dist += 2 * pi * radius * ( angle / (2 * pi) );
        
        return *this;
    }

    walk &backtrack(){
        std::tuple<int, int> tmp = _last;
        double tmp_dist = _last_dist;

        _last = _current;
        _current = tmp;

        _last_dist = _dist;
        _dist += _dist - tmp_dist;
        return *this;
    }
...
int main()
{
    ...
    walk e;
    assert(e.line({1.5, 2}).distance() == 2.5);
    return 0;
}
