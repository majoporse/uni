...
# + id="5g33Zria3Ajf"
mapping = define_mapping(inputs = ['X'], outputs=['F'])
inverse_mapping = define_mapping(inputs = [], outputs = [])
...
# + id="UIYzx6tk3Ajl"
s2 = 'č'
s2.encode(encoding='ascii', errors='strict')
...
# **Task 8 (Bonus)**: A text message (`str`) underwent the following operations:
#
# 1. It was encoded into `bytes` using the 'UTF-8' encoding.
# 2. The resulting bytes were encoded using the Base64 encoding (again into `bytes`).
# 3. The bytes were decoded into an ASCII string (`str`).
# 4. The string was converted into a list of individual characters (`list[str]`).
# 5. Each character in the list was converted to an integer using the `ord` function.
# 6. Each integer in the resulting list was converted to a string using the `str` function.
# 7. The list of strings was reduced using `join` to a single dot-delimited `str`.
#
# Can you decode the message?

encoded = '85.50.86.106.100.88.74.112.100.72.107.103.89.110.107.103.98.50.74.122.89.51.86.121.97.88.82.53.73.80.67.102.107.89.52.61'
...
def secret_inverse_mapping(key):
    ...
    char_codes = list(range(len(alphabet)))
    rotated_codes = char_codes[key:] + char_codes[:key]
    return define_mapping(rotated_codes, alphabet) #TODO replace both [] appropriately
...
key = 12
mapping = secret_mapping(key)
encrypted_character = mapping('a')
inv = secret_inverse_mapping(key)
dec = inv(encrypted_character)
print(dec)
...
def basic_decrypt(ciphertext: list[int], key: int) -> str:
    ...
    return [decryption_mapping(character) for character in list(ciphertext)]
...
# + id="6yccR0yR3AkA"
alphabet = string.ascii_lowercase + string.ascii_uppercase + string.digits + ' .'
a = basic_encrypt(alphabet, 0)
a2 = basic_encrypt(alphabet, 1)
a3 = basic_encrypt(alphabet, 2)
...
pt = bytes.fromhex('11'*0) # TODO change the plaintext
...
nonce1 = bytes.fromhex('00') # TODO change the nonce
nonce2 = bytes.fromhex('00') # TODO change the nonce
